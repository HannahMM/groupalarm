...
![](/img/zusatzmodule_imexport_export_herunterladen.png?classes=shadow)
...
![](/img/zusatzmodule_imexport_dynamischer_export.png?classes=shadow)

Alle Standarddaten sowie optionale Datenfelder werden hier aufgelistet. 

Folgende Konventionen sind dabei zu beachten:

- **Sprache**: Die Sprache wird gemäss nachstehender Konvention als Zahl
codiert: 1 = Deutsch, 2 = Französisch, 3 = Italienisch,  4 = Englisch
...
Export mit dem Rautezeichen \# gekennzeichnet.
 ...
 Vor Telefonnummern, bei denen die Prioritätsstufe (siehe dazu Optionales
Modul VoicePrio-Mobile) bestimmt wurde, stehen die folgenden Symbole:

   - 1!= Voice Prio GSM, Blitz +++


   - 2!= Voice Prio GSM, Sofort ++
  

   - 3!= Voice Prio GSM, Vorrang +
...
![](/img/zusatzmodule_imexport_optionale_felder_pro_teilnehmer.png?classes=shadow)
...
an den angelegten optionalen Feldern etwas zu ändern oder diese zu ergänzen, nehmen Sie dies im [Admin-Bereich](/de/admin/datenfelder/) vor {{% /panel %}}
...
![](/img/zusatzmodule_imexport_gruppen.png?classes=shadow)



### Ereignis mit Objekten


**Dateiname**: import_ev.csv

Hier können Sie Ihre erstellten Ereignisse exportieren.

![](/img/zusatzmodule_imexport_ereignis_mit_objekten.png?classes=shadow)

 - **Wurzel und Organisation**: Gibt an, welcher Wurzel bzw. Ordnern das Ereignis zugeordnet ist.
 - **Ereignis-Code und Ereignis-Name**: Name und Code des Ereignisses wie festgelegt
 - **Beschreibung, Text, Vorlage**: Die zugewiesene Beschreibung, Text und Name der Vorlage
...
Haben Sie im Admin-Bereich [Einsatzpläne](/de/admin/einsatzplaene/) erstellt und diese Gruppen zugeordnet, können Sie diese Zuordnung hier exportieren.

![](/img/zusatzmodule_imexport_gruppen_mit_einsatzplaenen.png?classes=shadow)
...
Haben Sie wie im Abschnitt [Zeitsteuerung verwalten](/de/mutieren/zusatzmodule/zeitsteuerungen/) Gruppen 
in Zeitsteuerungen eingeteilt, können Sie diese hier exportieren.

![](/img/zusatzmodule_imexport_gruppen_in_zeitsteuerungen.png?classes=shadow)
 ...
 - **LID und Name**: Schlüssel und Name der zugehörigen Gruppen, siehe [Gruppen](/de/mutieren/zusatzmodule/importexport/verfuegbare_reports/#gruppen)
...
![](/img/zusatzmodule_imexport_hierarchiebaum.png?classes=shadow)
...
![](/img/zusatzmodule_imexport_teilnehmer_in_einheiten.png?classes=shadow)
...
![](/img/zusatzmodule_imexport_tts.png?classes=shadow)
...
können Sie die Reihenfolge der Datenfelder nach Ihren Wünschen sortieren. Anschließend können Sie die Liste als PDF <img src="/img/als_pdf_speichern.png" alt="pdf" style='vertical-align:middle;display:inline;margin:0px 5px; '>
oder CSV Datei <img src="/img/als_csv_speichern.png" alt="csv" style='vertical-align:middle;display:inline;margin:0px 5px; '> herunterladen oder die Liste als Report speichern <img src="/img/reportvorlage_speichern.png" alt="report" style='vertical-align:middle;display:inline;margin:0px 5px; '>.

![](/img/zusatzmodule_imexport_telefonliste.png?width=800px&classes=shadow)
...
![](/img/zusatzmodule_imexport_telefonliste2.png?classes=shadow)


### Alle Alarmobjekte

**Dateiname**: import_oi.csv

Hier können Sie eine Liste mit allen Alarmobjekten , d.h. Alarmvorlagen, Konferenzvorlagen, Einheiten, Drehbücher herunterladen.

![](/img/zusatzmodule_imexport_alle_alarmobjekte.png?classes=shadow)

 - **Typ**: Art des Objektes. template = Alarmvorlage, conference = Konferenzvorlage, group = Wurzeln/Ordner, 
 scenario = Drehbuch, event = Ereignis
 
 - **ID**: Die jeweilige ID des Objektes (CID, LID oder zugeordnete Nummer)
 
 - **Name, Beschreibung**: Name des Objektes und, falls vorhanden, die Beschreibung

 - **Alarmtext**: Der standardmäßig festgelegte Alarmtext bzw. dessen Textcode
